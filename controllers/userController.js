...
exports.postUserlogin = async (req, res)=> {
  ...
  try{
      ...
      if(!email || !password){
          return res.status(422).json({error: "Plz filled the field properly from API"})
      }
      ...
      if(userlogin){
          ...
          if(!ismatch){
              ...
              return res.json({message: "Login successfully from API", status : 1 , token :  token });
          }
      } else{
...
exports.getdata =  async (req,res,next)=>{
  ...
  try{
      ...
      if(userauth){
        ...
        return res.json({message: "user data getted successfully.",data2:req.rootUser ,token: token, data: userauth });
...
else
{
   ...
   const addGiftcard = await Giftcard.create(newGiftcard);
    ...
    return res.json({msg : 'giftcard added from API',data:req.rootUser})
}
  }
   catch (error) {
   return res.json({msg : error})
